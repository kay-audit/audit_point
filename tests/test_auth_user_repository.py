...
class TestGetUserContextRedisCache:
    ...
    async def test_cache_hit_skips_repositories(self, repo, mock_conn):
        ...
        redis.get_json.assert_awaited_once_with("cache:userctx:12345:v0")
    ...
    async def test_cache_miss_queries_and_writes_through(self, repo, mock_conn):
        ...
        redis.set_json.assert_awaited_once_with(
            "cache:userctx:12345:v0", self._EXPECTED, ex=300,
        )
    ...
    async def test_epoch_read_failure_skips_cache_write(self, repo, mock_conn):
...
class TestUserContextInvalidationRace:
    """Устаревший контекст не должен воскресать из кэша до истечения TTL.

    Redis настоящий (fakeredis из autouse-фикстуры) — нужна реальная
    семантика INCR общей эпохи пользовательских кэшей.
    """

    async def test_write_after_invalidation_lands_in_dead_key(
        self, repo, mock_conn, fake_redis,
    ):
        ...
        async def _fetch(*args, **kwargs):
            fetches.append("sql")
            if len(fetches) == 1:
                # «Медленный» SELECT ролей ещё не вернулся, а админ уже сменил роль
                await invalidate_user_roles_cache("12345")
                return [{"name": "Админ"}]
            return [{"name": "Аудитор"}]

        mock_conn.fetch = _fetch

        first = await repo.get_user_context("12345")
        second = await repo.get_user_context("12345")

        assert first["roles"] == ["Админ"]
        # Следующий читатель видит актуальные роли, а не кэш прошлой эпохи
        assert second["roles"] == ["Аудитор"]
        assert len(fetches) == 2

        stale = await fake_redis.get_json("cache:userctx:12345:v0")
        assert stale["roles"] == ["Админ"]  # мёртвый ключ, его никто не читает
        fresh = await fake_redis.get_json("cache:userctx:12345:v1")
        assert fresh["roles"] == ["Аудитор"]
