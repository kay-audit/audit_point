...
- **Разбор ответа.** Тело `final`-конверта нормализуется до валидации (`_normalize_completion_payload`): дозаполняются `id` (GigaChat его не присылает), `object`/`created`/`model`, `choices[].index`/`finish_reason`/`message.role`. Присланное бэкендом не перетирается. Без этого штатный ответ GigaChat падал бы как «не соответствует схеме» — прямой HTTP-маршрут этого не замечает, потому что openai-SDK разбирает ответ ленивым конструктором. Невалидное тело даёт `BridgeSchemaError` (502), а 429/5xx от бэкенда и `finish_reason="error"` — `BridgeBackendError`: в обоих случаях переход на следующий маршрут — да, **retry — нет** (разбор детерминирован, а 429/5xx воркер уже повторял сам — иначе выходило бы до 15 вызовов LLM на сообщение). См. §6a в [`redis-llm-bridge.md`](../integrations/redis-llm-bridge.md).
- **Сбой Redis: до заявки ≠ после заявки.** `XADD` и цикл поллинга обёрнуты в **разные** `try`. Падение на `XADD` (конверта в стриме нет) — обычный `APIConnectionError`, ретраится как connect-класс, дубля не будет. Падение на `XRANGE` уже после успешного `XADD` — `BridgePollError` (подкласс `APIConnectionError`), и он в `_NEVER_RETRY_EXC`: воркер в этот момент исполняет первый конверт, а повтор положил бы второй с новым `request_id` — то есть второй запрос к LLM. Fallback при этом работает как обычно.
...
- `target_health` маршрут не исключает, а опускает в конец плана (ложноотрицательный health-check воркера не должен лишать пользователя единственного провайдера); при открытом breaker'е primary из плана **исключается** — но только если есть чем его заменить, единственный маршрут остаётся. Состояние breaker'а спрашивается лишь тогда, когда ответ может изменить план: `is_open()` сам переводит `open → half_open` по таймеру, поэтому лишний вызов испортил бы состояние.
...
| `BridgePollError` (redis-мост) | **Нет** | Сбой Redis на поллинге ответа, когда заявка уже в stream: повтор положил бы второй конверт, пока воркер исполняет первый. Fallback срабатывает (подкласс `APIConnectionError`). Сбой на самом `XADD` остаётся ретраибельным `APIConnectionError` |
...
Настройки — группа `CHAT__TEXT_ACTIONS__*` (`ChatDomainSettings.text_actions`): `CORRECTOR_MODEL` / `FORMALIZER_MODEL` (`None` → модель выбранного маршрута), три температуры, `PER_CALL_TIMEOUT_SEC` (60), `MAX_INPUT_CHARS` (32768). Пустой ввод, превышение `MAX_INPUT_CHARS` и неизвестный `mode` → `TextActionValidationError` (422). Поле `recommendations` формализатора — дисплей-онли подсказка аналитику, в карточку нарушения и экспорт не идёт.

#### Бюджет ответа и таймаут считаются от длины ввода

Ворота ввода подняты до 32768 символов, а вызов уходил без `max_tokens` и с жёстким таймаутом в 60 секунд — оба значения не зависели от объёма работы, и длинный текст ломался молча: у провайдера с низким дефолтным потолком completion'а ответ обрезался на середине и подставлялся пользователю как готовый, а там, где потолок высокий, вызов не укладывался в минуту и уходил в `APITimeoutError` — с пятью добросовестными повторами заведомо провального запроса.

Расчёт — чистые функции в `app/domains/chat/services/text_actions/budget.py`; там же константы с обоснованием каждой. Ручками `CHAT__*` они сознательно не сделаны: это не политика, а физика (как русский текст ложится в токены и как быстро генерирует модель), а каждая новая ручка заводится синхронно в трёх env-файлах и в реестре переменных.

| Величина | Как считается |
|---|---|
| Оценка длины в токенах | `CHARS_PER_TOKEN = 2.5` — сознательно консервативно для русского byte-level BPE: ошибка в эту сторону даёт запас бюджета, в обратную — обрезанный ответ |
| `max_tokens` | `estimate_tokens(len(text)) × ratio + 256`, зажато в `[512, 16384]`. `ratio` — по профилю действия |
| Таймаут одного вызова | `15 сек + max_tokens / 30 ток-сек`, но **не меньше** `PER_CALL_TIMEOUT_SEC`. Скорость генерации взята по нижнему краю наблюдаемого диапазона 30–60: таймаут обязан переживать медленный день провайдера |

**Два профиля.** `PROFILE_REWRITE` (ratio 1.15) — переписывающее действие, ответ сопоставим со входом; запас 15 % на то, что правка бывает длиннее исходника (восстановленная пунктуация у корректора, разбитые предложения у улучшайзера). Им идут **оба** режима `correct`. `PROFILE_EXTRACT` (ratio 0.6) — формализатор: его вызовы отдают JSON-выжимку, структурно короче текста, и полный переписывающий бюджет тут не только не нужен, но и вреден — `max_tokens` больше остатка контекстного окна провайдеры отдают ошибкой 400. Неизвестный профиль трактуется как переписывающий (ошибиться в сторону большего бюджета безопаснее).

**Бюджет сокращающего действия не урезан.** У режима `readability` («Пиши, сокращай») ответ по замыслу короче входа, и напрашивается дать ему меньший `max_tokens`. Так делать нельзя: `max_tokens` — это потолок, а не заказ длины. Модель сокращает по смыслу промпта, а не по потолку; заниженный потолок не сделает ответ лучше, зато при неудачной попытке (модель начала переписывать полно) обрежет его на середине. Экономии тоже нет — платится фактически сгенерированное. Поэтому оба режима `correct` идут по переписывающему профилю.

**`PER_CALL_TIMEOUT_SEC` — нижняя граница, а не жёсткое значение.** Настройка защищает короткое выделение от таймаута меньше привычного; сверху расчётная часть ограничена `MAX_ESTIMATED_TIMEOUT_SEC` (≈9.5 минут — следствие капа `MAX_OUTPUT_TOKENS`, дольше ждать нечего). То есть на предельном вводе один вызов легально идёт до ≈9 минут, и это не зависание.

**Оборванный ответ — ошибка, а не текст.** В акт аудита нельзя вставлять текст с потерянным хвостом, а отличить его от готового пользователь не может. Два независимых детектора, оба → `TextActionUnavailableError` (503, код `text-action-unavailable`):

1. **Транспортный, по факту от провайдера.** `finish_reason == "length"` в ответе (`llm_utils._raw_call`) — модель упёрлась в потолок. Работает для всех действий, включая сокращающие, потому что это факт, а не догадка по длине.
2. **Эвристика по доле объёма** — на провайдеров, которые `finish_reason` не сообщают. `budget.looks_truncated`: ответ короче 70 % исходника (`TRUNCATION_MIN_KEEP_RATIO`) при исходнике от 200 символов (`TRUNCATION_GUARD_MIN_CHARS` — на коротком выделении обрыв невозможен, а разброс доли велик). Применяется **только** к режиму `fix`: его промпт прямо запрещает сокращать, поэтому потеря трети объёма — это потерянный хвост, а не хорошая работа. Для `readability` короткий ответ — штатный результат, и проверка к нему неприменима.

**Ретраи капнуты двумя попытками** (`budget.MAX_ATTEMPTS_CAP`, применяется через `retry_attempts(r.max_attempts)` в обоих сервисах). Общий `CHAT__RETRY__MAX_ATTEMPTS` (5) писан под вызовы чата; здесь пользователь ждёт синхронно, таймаут вызова растёт с объёмом, и пять попыток на предельном тексте — это до трёх четвертей часа ожидания и пятикратно оплаченный prompt на запросе, который уже показал, что не проходит; у формализатора цена ещё умножается на шесть вызовов. Именно кап, а не подмена: заданное меньшее значение уважается. Общий слой `retry_on_transient` при этом не менялся — см. [`../testing/retry-test-scenarios.md`](../testing/retry-test-scenarios.md).

Тесты — `tests/domains/chat/test_text_actions_budget.py` (чистые функции), `test_text_actions_llm_utils.py` (`finish_reason`), `test_text_corrector_service.py` и `test_violation_formalizer_service.py` (проводка бюджета в вызов).
...
| `processing` | рост `reasoning_len`, изменение `answer_updated_at`, reclaim в `pending` (не более 3 эпизодов за всю жизнь подписки) | `ANSWER_TIMEOUT_SEC` (1800 сек) |

Первое наблюдение `answer_updated_at` ставится как baseline (без продления активности). Начиная со второго наблюдения — признак жизни любое изменение `answer_updated_at` (сравнение `!=`), в любую сторону: и обновление вперёд, и откат назад (агент сбросил `updated_at` при reclaim'е — см. строку выше, reclaim сам по себе признак жизни). Не продлевает таймаут только полное исчезновение строки-ответа.
...
**Schema/миграции.** Колонка `status` в `chat_messages` обеих СУБД — `VARCHAR(20) NOT NULL DEFAULT 'complete'` + `CONSTRAINT check_chat_messages_status_values CHECK (status IN ('streaming','complete','failed'))`. На PG — partial-индекс `idx_{PREFIX}chat_messages_streaming` (`WHERE status='streaming'`) для быстрого recovery; на GP 6.x (PG 9.4) partial-индексы не поддерживаются — обычный `idx_{PREFIX}chat_messages_status` на `(conversation_id, status)`. Колонка `agent_ref VARCHAR(36)` — там же. Всё создаётся стартовым `create_tables_if_not_exist`. Идемпотентные добавления `status`/`agent_ref` вшиты DO-блоками в сами файлы `app/domains/chat/migrations/{postgresql,greenplum}/schema.sql`, но они срабатывают, только когда схема домена вообще исполняется — то есть когда хотя бы одной таблицы чата в БД нет. На полностью развёрнутой БД такие блоки не отрабатывают: расширение доносится файлом миграции ([`database.md`](database.md) §6.5.5).
