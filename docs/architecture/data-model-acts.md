...
| `ViolationImageBlockSchema` | `id`, `type: "image"`, `image_id`, `caption`, `filename`, `width` | Картинка по ссылке: `image_id` — id строки `act_images` (uuid4-строка; пустая строка — черновик без картинки). Байты в контенте акта НЕ живут, формат и размер проверяются один раз при загрузке (`POST /acts/{act_id}/images`). `caption` — rich-HTML; `image_id`/`filename` — plain/verbatim; `width` 0–100 (% полезной ширины страницы, `0` — авто) |
...
Лимит ЧИСЛА блоков — на поле (`ACTS__IMAGES__MAX_ITEMS_PER_VIOLATION`, см. выше); лимиты БАЙТ (на файл и на акт) живут в таблице `act_images` и проверяются на загрузке, а не при сохранении контента (см. «Картинки нарушений» ниже). При сохранении rich-содержимое (`text.content`, `image.caption`) санитизируется `sanitize_rich_html` (nh3; текстблоки — bleach, общий allowlist `ACTS__SANITIZER__*`); `image_id`/`filename` — plain, не трогаются; ячейки table-блоков — verbatim. Деталь санитайзера — §13, deep-dive §9.3/§15 в [`textblock-editor-architecture.md`](textblock-editor-architecture.md).

#### Картинки нарушений — таблица `act_images`

Байты картинок вынесены из JSONB-полей `act_violations` в отдельную таблицу: снимки версий (`act_content_versions`, до 50 штук) хранили бы каждую картинку 51 раз, а `PUT /content` гонял бы мегабайты base64 при каждом автосохранении.

| Колонка | Тип | Смысл |
|---|---|---|
| `id` | `VARCHAR(36)` | uuid4-строка; на неё ссылается `image_id` блока |
| `act_id` | `INTEGER` (PG) / `BIGINT` (GP) | Акт-владелец |
| `content_hash` | `VARCHAR(64)` | SHA-256 сырых байт (hex) — дедупликация |
| `mime_type` | `VARCHAR(100)` | Реальный MIME, выведенный из байт (Pillow) |
| `byte_size` | `INTEGER` | Размер, `CHECK (byte_size > 0)` |
| `data` | `BYTEA` | Байты (прецедент BYTEA на GP — `chat_files.file_data`) |
| `created_by`, `created_at` | `VARCHAR(50)`, `TIMESTAMP` | Кто и когда загрузил |

**Дедупликация — `UNIQUE (act_id, content_hash)`, в пределах ОДНОГО акта.** Это не только совместимо с `DISTRIBUTED BY (act_id)` на GP, но и правильно по безопасности: глобальный дедуп дал бы одну строку на два акта, и пользователь без доступа ко второму получил бы её содержимое через первый. `ON CONFLICT` на GP недоступен — гонку двух параллельных загрузок одного файла ловит `UniqueViolationError` с повторным поиском по хэшу. GP: `PRIMARY KEY (id, act_id)`, `DISTRIBUTED BY (act_id)`, `WITH (appendonly=false)`.

**Эндпоинты** (`app/domains/acts/api/images.py`):

- `POST /api/v1/acts/{act_id}/images` — multipart-загрузка, ответ `{image_id, byte_size, mime_type, width, height}`. Требует прав на редактирование И владения блокировкой акта (те же требования, что у `PUT /content`). Здесь и только здесь проверяются `ACTS__IMAGES__MAX_FILE_SIZE` и `ACTS__IMAGES__MAX_TOTAL_SIZE_PER_ACT` (суммарный вес — запросом к `act_images`, не обходом JSONB). MIME определяется по САМИМ БАЙТАМ (Pillow), заявленный клиентом `Content-Type` контрактом не является.
- `GET /api/v1/acts/{act_id}/images/{image_id}` — байты. Адресация парой `(act_id, image_id)`: знание чужого uuid доступа не даёт. MIME — реальный, но только из `ACTS__IMAGES__ALLOWED_MIME_TYPES`, иначе `application/octet-stream`; всегда `X-Content-Type-Options: nosniff` (картинка рендерится в `<img src>` на origin'е приложения — произвольный MIME означал бы XSS через загруженный SVG/HTML). `Cache-Control: public, max-age=31536000, immutable` — содержимое адресуется идентификатором и не меняется.

**Сжатие на фронте — до отправки.** Байты, которые уедут на сервер, готовит `violation-image-resize.js::downscaleImage` (вызывается из `violation-blocks.js::insertImageFilesInOrder`, файлы обрабатываются последовательно — пачка параллельных POST'ов упёрлась бы в пул соединений ПРОМ-учётки). При вставке пачки пользователь выбирает режим в диалоге; последний выбор запоминается в localStorage (`violation_image_quality_mode`). Режимов три — `RESIZE_PRESETS`:

| Режим | Кнопка | Длинная сторона | Качество | Целевой формат |
|---|---|---|---|---|
| `high` | «Сжатие» (по умолчанию) | 1920 px | 0.9 | WebP |
| `medium` | «Среднее» | 1400 px | 0.8 | WebP |
| `original` | «Исходное» | 4096 px | 0.95 | **свой же формат файла** |

Целевой формат `high`/`medium` — WebP, а не JPEG, намеренно: акты состоят из скриншотов банковских систем с мелким текстом, блочный DCT и chroma subsampling JPEG дают вокруг букв ореолы, а WebP на 25–35 % меньшем весе держит края символов и умеет прозрачность. Браузер без WebP-энкодера получает JPEG-фолбэк, и там полупрозрачная картинка остаётся оригиналом (JPEG схлопнул бы альфу). `high` держит 1920 px, чтобы типичный FullHD-скриншот не ресемплился вообще: дробный коэффициент масштабирования размывал бы мелкий шрифт сильнее, чем сама перекодировка.

**`original` — не «ничего не делать», а «не терять деталей и не менять формат».** Режим тоже перекодирует картинку, но в её собственный MIME (PNG → PNG, JPEG → JPEG), с мягкими параметрами: 4096 px по длинной стороне — выше типичного 12-Мп телефонного фото, ресемплится только настоящий outlier вроде RAW-скана; качество 0.95 — почти без потерь там, где формат вообще lossy. Для PNG канал quality `toBlob` игнорирует (формат lossless), поэтому решает **размерный гейт: результат принимается, только если он легче оригинала** — и в этом режиме гейт строгий безусловно, даже если предохранительный cap по стороне что-то ужал. Причина в том, что уже хорошо пожатый исходным энкодером PNG после `canvas.toBlob('image/png')` вполне может распухнуть; «Исходное» не должно ухудшать файл ни при каких условиях, поэтому такой результат отбрасывается и уходит оригинал байт-в-байт.

GIF не пережимается ни в одном режиме — canvas отдал бы только первый кадр и убил анимацию. Любой сбой `createImageBitmap`/`toBlob` деградирует к оригиналу. Чистая логика (`resolveResizeMode`/`shouldDownscale`/`computeScaledSize`/`hasTransparentPixels`/`resolveActualFilename`) покрыта `tests/js/violation-image-resize.test.mjs`; сам canvas-конвейер в jsdom не воспроизводится.

Серверных гарантий это не заменяет: `ACTS__IMAGES__MAX_FILE_SIZE` и MIME по байтам проверяются на загрузке в любом случае (см. выше).

**Сборка мусора.** `ActContentVersionRepository._collect_image_garbage` (зовётся из `_cleanup_old_versions`, там же, где вытесняются старые версии) удаляет картинки акта, на которые не ссылается ни актуальный контент, ни ОДНА из уцелевших версий. Две страховки от ложного удаления: картинки моложе `GC_MIN_AGE_MINUTES` (60) не трогаются вовсе — загруженная, но ещё не сохранённая в контент картинка неприкосновенна; непонятая структура JSONB пропускается, а не считается «ссылок нет».

**Экспорт.** Форматтеры в БД не ходят (DOCX собирается в пуле потоков): `ExportService._load_images` подгружает байты нужных картинок ОДНИМ запросом до рендера и передаёт картой `image_id → {data, mime_type}` (`ExportContext.images` для DOCX, ключ `images` в data-dict для MD). DOCX встраивает байты напрямую, а WebP (python-docx его не умеет) перекодирует в PNG через Pillow. MD восстанавливает data-URL: экспорт — один файл, который пользователь уносит с собой, ссылка на API протухла бы вне сессии. TXT картинки не выводит вовсе (только имя файла), поэтому байты ему не подгружаются.

Семь текстовых полей нарушения (`violated`/`established`/`description`/`reasons`/`measures`/`consequences`/`responsible`) можно автозаполнить из свободного описания — кнопка
...
`POST /api/v1/chat/text-actions/formalize-violation`): конвейер D17 в две стадии — 4 экстрактора
разбирают текст параллельно, затем параллельно же сборщик отчёта (`VIOLATION_SYSTEM`) переписывает
извлечённое в официально-деловой текст, а промпт рекомендаций считает, чего не хватает; раунда ожидания
вторая стадия не добавляет. «Нарушено» и «Описание» берутся прямо у экстрактора сути (норматив и
метрики), остальные пять полей — у сборщика, а при его сбое собираются напрямую из экстракторов в той же
строковой форме. Списки применяются ТОЛЬКО в «Описании»: метрики побулитно расшифровывают сказанное в
«Установлено», прочие поля — связный текст, а пришедшие массивом лица склеиваются через «; ».
Что не извлеклось — контейнер не трогается; уже
заполненное поле пустым ответом не затирается. CodeMining/ProcessMining формализатор не заполняет — гарантия держится составом полей `FormalizeResponse` (фронт применяет пересечение ключей ответа с реестром, `formalizedFieldKeys`). Каждое непустое значение ответа уходит НОВЫМ text-блоком в конец своего поля (`ViolationManager._applyFormalized`, `violation-core.js`) с автовключением чекбокса поля. Контракт ответа: ВСЕ поля — готовый безопасный HTML (бэкенд экранирует скаляры + `\n`→`<br>`, «Описание» — `<ul><li>`), фронт только санитизирует `SafeHTML.sanitize(value, 'acts')` — эвристики «HTML или plain» нет. Провал всех экстракторов → 503 `TextActionUnavailableError` («ИИ-сервис недоступен»), а не пустой успех; частичный провал даёт частичный результат. Заголовок панели
...
| `ACTS__INVOICE__HIVE_REGISTRY_SCHEMA` | `s_grnplm_ld_audit_da_project_4` | Где лежит реестр Hive-таблиц |
...
- **Поверхности уведомлений зависят от уровня** (решение: warning не должен шуметь, error приравнен к фактуре):
  ...
  - **Список замечаний обоих уровней** виден внутри акта в колокольчике конструктора (`static/js/constructor/header/notifications-source-validation.js`, читает `validation_issues` последнего сохранения). Внутри акта живые замечания **read/delete не поддерживают** (всегда напоминают) — контекстное меню записи пустое («Нет доступных действий»). Три табличных кода (`table_no_data`, `table_no_header`, `table_empty_header`) этот источник **не показывает** — их считает и группирует живой источник «tables» (константа `SUPPRESSED_CODES`), иначе каждое такое замечание выводилось бы дважды: сгруппированно и плоско. На серверный статус акта скрытие не влияет — он считается по полному набору замечаний.
  ...
  - **Рамка недозаполненной таблицы на листе — пастельная** (`--warning-light`, 1px): неполнота (таблица без данных) не ошибка. Яркая рамка (`--error`) — у того, что ломает экспорт: структурный дефект сетки, а также таблица без строки заголовка / с пустыми заголовками (их же гейтит `_validateForExport` через `ValidationTable.validateHeaders`, и сервер отдаёт по ним severity='error').
...
**Санитайзер — `utils/html_sanitizer.py`.** Единый цикл `_sanitize_violation_common` (общий для obj- и dict-формы) — по всем 10 полям реестра → по блокам поля → диспетчер `_sanitize_block` по `type`: `text` → `content` через `sanitize_rich_html` (nh3); `image` → `caption` через `sanitize_rich_html`, `image_id`/`filename` — plain, не трогаются; `table` — не трогается (ячейки хранятся дословно, тот же инвариант B8, что у больших таблиц узла). Отсутствующее/`None`-поле или `blocks` не-список пропускаются без исключения.
...
**DOCX — `formatters/docx/builders/violation.py::build_violation`.** Тот же цикл по `ordered_fields` и те же предикаты видимости/метки: размер шрифта поля — `Sizes.violation_pt` при `small=True`, иначе `Sizes.body_pt` (вместо позиционного хардкода «первые четыре поля мелкие»); у поля С МЕТКОЙ первый text-блок идёт inline с ней («Метка: текст» — привычный вид), остальные блоки — своими абзацами/shape'ами. Исключение — text-блок, НАЧИНАЮЩИЙСЯ со списка: метка тогда остаётся самостоятельным абзацем, а список идёт следом. Иначе абзац с меткой сам получал бы `w:numPr` первого пункта, и в Word выходило «• Причины: текст пункта» — служебное слово читалось как часть маркированного пункта (развилка — в `render_block_segments`, ветка `label_only`; превью повторяет её предикатом `startsWithList`, MD/TXT метку не инлайнят вовсе); у поля с `labeled=False` абзац-метка не создаётся вовсе (в том числе пустой, когда поле начинается с картинки или таблицы) — все блоки рендерятся напрямую; `image` → `_add_image`/`_scale_picture`, `table` → общий `build_table` через `EmbeddedTableSchema`. Списки (`<ul>/<ol>/<li>`) получают собственную нумерацию — каждый элемент списка свою пару `w:abstractNum`+`w:num` (`docx/numbering.py::create_list_num`), вложенность передаётся через `w:ilvl`, а не built-in стиль абзаца Word — на общем пути `render_block_segments` (`docx/builders/inline.py`) — рендерятся одинаково и из текстблоков, и из любого rich-поля нарушения (§9.3/§15 в [`textblock-editor-architecture.md`](textblock-editor-architecture.md)).

**Фронт: контейнер блоков и аудит.** `violation-blocks.js::createBlocksField` — единый компонент секции ОДНОГО поля (заголовок/чекбокс, тулбар «+ Текст | + Таблица | + Картинка», зона вставки с контекстным меню и приёмом файлов) для всех 10 полей реестра — «Дополнительный контент» перестал быть особым случаем. Записи в модель — только через мутаторы `violation-mutations.js` (`setFieldEnabled`/`setFieldOrder`/`setBlockField`/`addBlock`/`removeBlock`/`removeBlocks`/`moveBlocks`); исключение — ячейки встроенных таблиц: их правит общая машинерия таблиц напрямую в `blocks[].table.grid` через резолвер `table-store.js` (запись ловит Proxy состояния, read-only отсекают гейты самой машинерии). `violation-audit.js::ViolationAudit` — снимок для дифф-аудита двухфазный: `synthesize()` кладёт фингерпринты (по реестру, блок-за-блоком; картинки — `image_id` плюс метаданные, таблицы — только содержимое ячеек) в статический `_pendingSnapshot` (не коммитит), `confirmSave()` промотирует его в `_snapshot` только ПОСЛЕ подтверждённого успешного сохранения (вызывается из `shared/api.js` — `saveActContent`/`forceSaveToDb` — и `constructor/lock-manager.js` — `_initiateExit` — в ветке успеха). Раньше снимок коммитился независимо от результата сохранения, что могло зафиксировать несохранённое состояние как базу для следующего диффа.
...
- **DOCX**: `DOCX_MAX_HEADING_LEVEL=9`. Ширина картинки нарушения настройкой НЕ задаётся: она считается в процентах полезной ширины страницы из поля `width` блока (`_scale_picture` в `docx/builders/violation.py`), потолок высоты — `ACTS__IMAGES__IMAGE_MAX_HEIGHT_PERCENT`.
...
- Эволюция структуры идёт через:
  1. SQL-миграции — изменения схемы таблиц (`migrations/postgresql/schema.sql`, `migrations/greenplum/schema.sql`); схема исполняется при старте через `create_tables_if_not_exist` (см. [`database.md §6.5`](../guides/database.md#65-миграции)). Легаси-данных не держим: ломающее изменение схемы означает пересоздание БД, расширяющее — правку `schema.sql` плюс файл миграции в `docs/migrations/` для уже развёрнутых БД.
