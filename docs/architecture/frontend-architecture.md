...
`--doc-line-height`, `--doc-readability-zoom`, `--doc-font-size-table-screen`,
`--doc-list-indent` — по другим правилам, чем обычные UI-токены: линейные
величины заданы в пунктах (12pt / 9pt / 36pt), а не в rem, и повторяют эталон
DOCX (`app/domains/acts/formatters/docx/styles.py`, `numbering.py`), а не
дизайн-шкалу интерфейса; безразмерные (`--doc-line-height` 1.15,
`--doc-readability-zoom` 1.25) — оттуда же, кроме зума: он чисто экранный и на
экспорт не влияет. Прежний UI-токен
...
- **Ячейки таблиц конструктора** — кегль `--doc-font-size-table-screen` (`typography.css`) = `calc(var(--doc-font-size-small) * var(--doc-readability-zoom))` = 9pt × 1.25 = 15px, а не `--font-size-sm` от плотности зоны: внутри `.items-container` (форма заполнения) тот давал 9.75px — МЕНЬШЕ печатных 9pt (12px), тогда как тело текстблока экранно на 25% КРУПНЕЕ печатного (та же пропорция, что у DOCX: `Sizes.table_data_pt`/`table_header_pt` = 9pt против `Sizes.body_pt` = 12pt). Множитель читаемости здесь применён к КЕГЛЮ через `calc`, а не CSS `zoom`, как у текстблока/rich-полей нарушений, — сознательно: ширины колонок считаются в JS долей от `table.offsetWidth` по смещению курсора (`constructor/table/table-sizes.js::startColumnResize`), а под `zoom` `offsetWidth` и `clientX` расходятся в разных системах координат (замер: жест на 100px под `zoom: 1.25` двигал границу колонки на 125px). Кегль на координаты resize не влияет — `calc` там, где `zoom` сломал бы перетаскивание. Гарнитура ячеек — тот же `--doc-font-family` (`table-base.css`), не интерфейсная.
