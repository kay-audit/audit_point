...
`--doc-line-height`, `--doc-list-indent` — по другим правилам, чем обычные
UI-токены: линейные величины заданы в пунктах (12pt / 9pt / 36pt), а не в rem,
...
`tests/test_document_typography_tokens.py`. Отдельный ratchet
...
- **Лист предпросмотра** — геометрия и тело заданы в px/pt/mm (`preview-page.css`) и от rem не зависят. Заголовки листа (`.preview h1…h6`) — исключение, они считаются от UI rem-токенов (`--font-size-xl/lg/base/sm`, `preview-typography.css`), поэтому зонные сжатия кегля на панель предпросмотра не вешают, но и от rem не защищены (известный долг: одна и та же разметка даёт разный кегль в конструкторе, корень 12px, и в диалоге версий, корень 13px). **Подпись таблицы** (`.preview-table-title`) структурно тоже `<h4>`, но из общего правила заголовков выведена намеренно: более специфичный селектор `h4.preview-table-title` (`preview-table.css`) фиксирует её на документном `--doc-font-size`, а не на UI-токене `--font-size-base` — см. §7.3 [`data-model-acts.md`](data-model-acts.md) (правило оформления подписи таблицы).
