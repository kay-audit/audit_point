...
- **Тело текстблок-редактора** — кегль и интервал заданы напрямую документными токенами `--doc-font-size` (12pt) / `--doc-line-height` (1.15, `textblock-content.css`), от rem-плотности и корня не зависят; читаемость на экране даёт CSS `zoom: var(--doc-readability-zoom)` (1.25), геометрию не меняющий. `.textblock-editor` в `density.css` по-прежнему переопределяет `--font-size-base/lg/xl` в px, но это осталось только для заголовков `h1-h3` внутри редактора — базовый кегль абзацев через этот токен больше не идёт.
