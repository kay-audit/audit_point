...
- **Превью текстблока — print-precise, supersedes B-22** (Task C, 2026-07):
  ...
  отступе.
- **Втяжка списков предпросмотра — та же дыра дважды, теперь закрыта на трёх
  поверхностях.** Правила `preview-typography.css` для `ol`/`ul`/`li`
  (втяжка, каскад маркеров по уровням) были селекторами вида `.preview ol`,
  а класс `.preview` висит ТОЛЬКО на inline-панели (`#preview`,
  `preview_panel.html`). Модальное меню предпросмотра (`#previewMenuBody`,
  `header_preview_button.html`) и диалог истории версий
  (`.version-preview-ui`, `portal/acts-manager/version-preview.js`)
  собираются теми же рендерами, но без этого класса — списки там попадали
  под глобальный сброс (`base/reset.css` обнуляет padding/margin всему):
  нулевая втяжка, маркер при `list-style-position: outside` уезжал за левый
  край контента (замер: `li.left == content.left`). Фикс — селекторы
  переписаны на `:is(.preview, .preview-sheet, .version-preview-ui)`; втяжка
  переведена в документный токен `--doc-list-indent` (36pt = `w:ind left`
  720 твипов на уровень, `docx/numbering.py::_LIST_INDENT_STEP`) — общий с
  редактором (`textblock-content.css`), вместо прежних rem-токенов
  (`--spacing-2xl` в превью / `--spacing-xl` в редакторе), плававших вместе
  с плотностью зоны (18px в конструкторе, 24px в inline-превью, 0 в двух
  других поверхностях). **Правило на будущее**: документную типографику
  нельзя вешать на класс ОДНОЙ поверхности предпросмотра — их минимум три
  (inline-панель / модальное меню / диалог версий), и `.preview` есть только
  у первой; новый селектор для листа/списка/блока предпросмотра обязан
  целиться в `:is(.preview, .preview-sheet, .version-preview-ui)`. Это уже
  второй случай ровно той же дыры — первый был с `line-height` (см.
