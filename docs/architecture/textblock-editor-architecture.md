...
**Базовый размер шрифта** — единый дефолт настроек в ПУНКТАХ
(`ACTS__TEXTBLOCKS__FONT_SIZE_DEFAULT`, дефолт **12pt**), он не хранится
per-block. Конвертации px→pt в базе экспорта больше **нет**: число из
тулбара доезжает до DOCX как есть (§6, §10); `_PX_TO_PT`/`_extract_size_pt`
(×0.75) остались только как страховка для внешней вставки (Word-paste и
...
`li`; `padding-left` у списков остаётся — в нём рисуется маркер). Это не
косметика: собственные отступы давали РАЗНЫЙ шаг в зависимости от того,
каким тегом выражен блок (замер: `<p>` — 34.4px против 18.4px у соседнего
`<div>`, заголовки — до 40px), а у списков отступ вдобавок рос вместе с
кеглем (задан в `em`). Редактор сам `<p>`/`h1`-`h6` не создаёт (Enter даёт
`<div>`) и ни один путь вставки их не приносит — Word-путь расплющивает
`p`/`div`/`li` в инлайн + `<br>` (`_flattenWordBlocks`, §7), внешний берёт
только `textContent`, свой буфер несёт `<div>`; но allowlist их пропускает,
поэтому они живут в контенте из прямого API и восстановленных версий.
Word печатает так же: `render_block_segments` зануляет `space_after` у всех
сегментов текстблока, 3pt достаётся блоку целиком (§10). Страж —
`tests/js/preview-print-geometry-parity.test.mjs`. Любое
изменение размера через тулбар материализуется как
`<span style="font-size:...pt">` внутри `content` (§6) — переживает
...
**Многоуровневость: Tab/Shift+Tab.** `_handleListTab` (`textblock-editor.js:1480`)
...
`_caretListItem(range, editor)` (`:1503`) — обёртка над `_listItemAncestor`
(`:1571`), разбирающая случай «range заякорен на элементе»: Chrome при
...
глубины Tab-ветка сама НЕ сравнивает — его держит гейт `execCommand` (ниже),
единственное место сравнения. На потолке клавиша ВСЁ РАВНО проглатывается
(`preventDefault` зовётся по факту «каретка в `<li>`»), но `indent` тихо не
идёт — иначе Tab на дне списка увёл бы фокус из редактора.

**Потолок глубины — настройка, а не константа.** UI-потолок задаёт
`ACTS__TEXTBLOCKS__MAX_LIST_LEVEL` (`acts/settings.py::TextblocksSettings`,
дефолт 4 = пятый уровень), фронт получает его через `GET /acts/limits` →
`getStructureLimits().maxListLevel` (фолбэк до ответа —
`AppConfig.limits.textblock.maxListLevel`). Читает значение
`_listLevelCeiling()` (`textblock-core.js:292`) — ВЫЗОВОМ в момент проверки, а
не константой на импорте: ответ `/limits` приходит асинхронно, да и
зафиксированное на module-level значение в графе с циклом импортов дало бы
`undefined`. Настройка прижимается `HARD_MAX_LIST_LEVEL = 8`
(`textblock-core.js:60`, НЕ экспортируется) — это девятый и последний уровень,
который умеет описать `w:abstractNum` в OOXML; настройка валидируется `le=8`
на бэке, а DOCX-сборщик (`inline.py`) клампит на `ilvl 8` независимо.

**Собственные `indent`/`outdent` вместо нативных.** Нативный `execCommand('indent')`
на одиночном пункте порождает список-СИРОТУ рядом с ним (без предыдущего
`<li>`), из которого каноническую форму `<li>текст<ol>…</ol></li>` без пустого
`<li>`-хоста не собрать: маркеры таких хостов выстраивались в одну строку
(«a) i. 1. Текст»), а `indent` соседнего пункта клал в `<li>` ВТОРОЙ
список-сиблинг, и его нумерация стартовала заново. Поэтому углубление делает
`_indentListItem(li)` (`textblock-editor.js:1625`), а нативная команда не
вызывается вовсе — ни для `indent`, ни для `outdent` (см. ниже):
предыдущего соседнего `<li>` нет → **no-op** (первый пункт уровня не
углубляется — поведение Word/ProseMirror); иначе пункт ЦЕЛИКОМ, со своими
вложенными списками, переезжает в подсписок предыдущего пункта — существующий
ТОГО ЖЕ типа (`_sublistHost`, `:1648`, нумерация продолжается) либо новый.

Команда применяется ко ВСЕМУ выделению, а не к одному пункту: набор считает
`_selectedListItems` (`:1531`) — задетые диапазоном пункты, из которых
оставлены «дно» выделения (пункт без задетых потомков) и пункты, накрытые
диапазоном целиком (такие едут со своим поддеревом, потомки из набора
выбывают). Иначе выделение внутри подпункта двигало бы весь родительский
пункт, а Tab по нескольким строкам — только первую (нативные команды двигали
всё выделение, и обе свои обязаны вести себя так же). Пункты обрабатываются
в порядке документа: следующий видит предыдущий уже переехавшим и попадает в
тот же подсписок; на подъёме порядок так же держит цепочку «хвост уехал в
подпункты — следующий выделенный пункт поднимается за предыдущим».

Каретка переживает переезд снимком ПЛОСКИМИ значениями (узел + смещение,
`_captureCaretRange`/`_restoreCaretRange`, `:1757`), а не клоном `Range`:
`Range` живой, и удаление узла (перенос `<li>` начинается именно с него) по
спеке переставляет границы всех живых диапазонов — включая клон — на место
удалённого узла в СТАРОМ родителе, т.е. каретка уезжала бы из пункта в
промежуток внешнего списка. Узлы же переезжают вместе с пунктом и остаются
валидными. Плата — нативный undo-стек Chromium своей мутации не
видит (Ctrl+Z шаг `indent` не откатит); стек и раньше был неточен, потому что
нормализатор мутировал DOM после `document.execCommand`.

**Подъём уровня: `_outdentListItem`.** Нативный `outdent` ронял пункт РЯДОМ с
подсписком (список-сиблинг) или прямо внутрь чужого `<li>` — форма «li внутри
li», где браузер рисует маркер посреди чужой строки, а DOCX/MD/TXT считают
пункт частью хозяина; нормализатор чинил это задним числом, уже после того как
пользователь увидел прыжок разметки. Своя реализация
(`textblock-editor.js:1678`) детерминирована и работает по трём правилам:
(1) **хвост пункта** — его следующие соседи по списку — обязан остаться на
своём уровне, поэтому переезжает в подсписок самого пункта (`_sublistHost`,
тот же, что у `indent`); (2) подсписок лежит в `<li>` (канон) → пункт встаёт
СЛЕДУЮЩИМ за этим `<li>` в внешнем списке; (3) список верхнего уровня → пункт
выходит из списка **абзацем** (`_listItemToParagraph`): свой контент — в
`<div>` сразу за списком, собственные подсписки — списками за абзацем, а
хвост (уже уехавший в подпункты на шаге 1) становится списком после него.
Получается чистый split: «до» остаётся в исходном списке, «после» — в новом,
порядок содержимого сохраняется. Опустевший список удаляется
(`_dropEmptyList` — остатки не теряются, а переезжают на его место); пустой
абзац получает `<br>`, иначе Chromium рисует его нулевой высоты и кареткой в
него не встать.

Носителем абзаца взят именно `<div>`, а не `<p>`: это то, во что Chromium
сам превращает Enter в `contenteditable`, поэтому вышедший из списка пункт
неотличим от обычной строки блока. Все потребители уже читают его как границу
абзаца: DOCX держит `div` в `_BLOCK_TAGS` и открывает им контейнер
(`formatters/docx/builders/inline.py:70,642`), MD/TXT меняют `</div>` на перенос
(`formatters/utils/html_utils.py:66`), оба санитайзера тег пропускают
(`sanitize.js::ACTS_TAGS_FALLBACK`, `html_sanitizer.py::_FALLBACK_TAGS`). То есть
выход из списка не требует правок ни в экспортерах, ни в allowlist'ах.

`indent` → `outdent` возвращает исходную разметку, а повторный `indent` снова
углубляет пункт — именно этот третий шаг ловит исходный баг (на форме
«`li` внутри `li`» `_indentListItem` молча отказывал) и закреплён и юнит-тестом,
и Playwright-сценарием.

**CSS-обход hit-testing Chromium для li-хозяев подсписков.** У «правильной»
вложенности (`<li>текст<ul>…</ul></li>`) Chromium ставил каретку в НАЧАЛО
текста li-хозяина при клике мышью правее конца его строки: точка попадает в
анонимный блок, оборачивающий инлайн-контент рядом с блочным `<ul>`, и
`PositionForPoint` отдаёт позицию 0 (нативная сиблинг-форма `<ul><ul>`, которую
строил браузерный indent, от этого не страдала — там `li` остаются листьями).
Обход — пара правил в `textblock-content.css`: `display: flow-root list-item`
на `li` плюс `display: inline-block; width: 100%; vertical-align: top` на
`li > ul/ol`; нужны ОБА разом — порознь часть конфигураций остаётся битой.
Диагностировать только живыми кликами: `caretRangeFromPoint` с обходом
рассинхронизирован (может отдавать 0 там, где реальный клик уже корректен).
Вторая плата — подсписок-инлайн-блок лежит в ОДНОМ строчном контексте с
текстом li-хозяина, поэтому его строка перестаёт быть последней в блоке, и
`justify` растаскивал слова на всю ширину колонки («Второй пункт      с
подсписком»); гасится точечно — `text-align: left` на `li:has(> ul, > ol)`,
с возвратом `justify` вложенным спискам (иначе `left` унаследовался бы
вглубь), а собственный `text-align` из `content` (TB-1) как inline-стиль
сильнее обоих правил. Третья — лишний зазор до следующей строки после
вложенной ветки (инлайн-блок добавляет свой line box). Регрессия —
сценарий «клик мышью правее конца текста» в спеке 29.

**Гейт `indent`/`outdent` — в ядре, не в тулбаре.** `execCommand`
(`textblock-core.js:317`) ДО исполнения команды проверяет для
`LIST_LEVEL_CMDS = ['indent', 'outdent']` (`:51`) два условия: (1) каретка
...
уровень (`_listLevel`, `textblock-editor.js:1588`) меньше потолка
`_listLevelCeiling()` — иначе пункт меню «Уровень глубже» углублял бы за
настроенный предел (и дальше уровня 8, который DOCX-сборщик молча клампит);
при выделении из нескольких пунктов потолок отсеивает упёршиеся в него, а
остальные углубляются.
Пройдя гейт, `indent` исполняется своим кодом (`_indentListItem`, выше) с тем
же стоком: нормализация → `saveContent`. Гейт
...
глубже»/«Уровень выше» `aria-disabled="true"` вне списка; «Уровень глубже»
гасится ТАКЖЕ на потолке глубины (тот же `_listLevelCeiling`), «Уровень выше»
на потолке остаётся активным — наверх выходить можно. Погашенные пункты
...
**Нормализация вложенности — страховка на чужую разметку.** Своим `indent`
валидная форма получается сразу, но правка может прийти мимо него — paste,
undo, внешний HTML, контент, сохранённый прежней версией: список прямо внутри
списка, минуя `<li>` (`<ul><li>a</li><ul><li>b</li></ul></ul>`), либо два
подсписка одного типа сиблингами в одном `<li>`.
`_normalizeListNesting(root)` (`:1812`) приводит разметку к валидной по
четырём правилам: (0) `<li>` прямо внутри `<li>` всплывает СЛЕДУЮЩИМ за своим
хозяином (артефакт нативного outdent, вставки внешнего HTML и старого
контента); группа таких пунктов поднимается с ОДНИМ якорем, иначе их порядок
перевернулся бы, а цепочка «li в li в li» распрямляется за один проход, потому
что снимок идёт в порядке документа и хозяин обрабатывается раньше вложенного;
(1) `ul|ol`, чей родитель `ul|ol`, переезжает внутрь ПРЕДЫДУЩЕГО
соседнего `<li>`; (2) список-СИРОТА (предыдущего `<li>` нет) **разворачивается
на уровень вверх** — на его месте остаётся его же содержимое; пустой
`<li>`-хост больше НЕ создаётся: его маркер рисовался отдельным пунктом (та
самая строка «a) i. 1. Текст») и давал фантомную строку в MD/TXT; (3) соседние
подсписки ОДНОГО типа внутри `<li>` сливаются в первый (`_mergeSublists`,
`:1861`) — иначе второй `ol` нумеровался бы с начала и в превью, и в DOCX;
разные типы (`ul` рядом с `ol`) не сливаются, это разные подсписки пункта.
Проход повторяется до стабилизации, но не больше `NORMALIZE_MAX_PASSES = 4`
(каждый проход строго уменьшает число нарушений — лимит лишь защита от
зацикливания на неучтённой форме). Идемпотентно, работает и на живом DOM
(каретка переезжает с поддеревом), и строкой на detached `<template>`
(`_normalizeListNestingHtml`, `:1888`). Две точки вызова: `execCommand` — на
своих командах уровня через общий сток `_commitListLevelChange`
(`textblock-core.js:306`), а после нативных `insertUnorderedList`/
`insertOrderedList` — веткой `LIST_STRUCTURE_CMDS` (`:66`), в обоих случаях ДО
`saveContent`; и БЕЗУСЛОВНО в `saveContent` (`:166-167`) — правка могла
...
проходом `_removeBlockFormat(root)` (`textblock-editor.js:1917`) сразу
после нативной команды: разворачивает `<ul>/<ol>` в `<br>`-разделённый
поток (`_unwrapListBlock`, `:2002` — содержимое `<li>` переносится на место
...
**`applyFontSize(fontSize)`** (`textblock-toolbar.js:356`):

1. Клампится по границам из `ACTS__TEXTBLOCKS__FONT_SIZE_MIN/MAX`
   (`getStructureLimits()`, читает `/api/v1/acts/limits`).
2. **Есть выделение** — расширяет диапазон за целые капсулы
   (`_expandRangeOutOfMarkers`), затем `_applySizeToRange` идёт
   **поузлово**: `_splitRangeEdges` режет краевые текстовые узлы по границе
   выделения, каждый целиком выделенный текстовый узел одевается в свой
   `<span style="font-size:...pt">` (`_sizeTextNode`); пункты списка
   (`<li>`), выделенные ЦЕЛИКОМ (весь видимый текст, включая подпункты —
   `_listItemsFullyInRange`), получают размер на сам `<li>`, а не на
   обёртку — от него наследует `::marker`. Капсулам внутри диапазона размер
   ставится безусловно (`contenteditable=false`, внешний span их не
   накрывает). Прежний способ (`range.extractContents()` в один
   обёрточный span) на выделении через несколько `<li>` расщеплял частично
   захваченные узлы и оставлял пустые пункты-огрызки по краям
   (`<ul><li></li><span><li>…</li></span><li></li></ul>`) с `<li>` внутри
   `<span>` — не понимали ни превью, ни DOCX-сегментатор.
...
**Per-tag политика для `style`** (TB-1): блочные теги `div`/`p`
...
полный CSS-allowlist выше без `text-align`. `<li>` — блок с ИСКЛЮЧЕНИЕМ:
сверх `text-align` ему разрешён и собственный `font-size` (клампится/px→pt
конвертируется тем же кодом, что у span, TB-6) — без этого исключения
размер маркера не переживал бы сохранение: `::marker` наследует кегль от
САМОГО `<li>`, а не от вложенного span, и DOCX печатает маркер меткой
абзаца тем же значением (`_set_paragraph_mark_size`, §10.1). Прочие
свойства (`color`/`background` и т.п.) пункту не нужны и срезаются — DOCX
их у `<li>` не читает, был бы шов превью↔экспорт. Зеркало: `_BLOCK_STYLE_TAGS`/
`_ITEM_STYLE_TAGS` на бэке ↔ `BLOCK_STYLE_TAGS`/`ITEM_STYLE_TAGS` во
фронте (`sanitize.js`). Причина асимметрии div/p против span: `font-size`
эмитится на span (Range-хирургия) или на сам `<li>` (§6), `text-align` —
...
единый дефолт настроек в пунктах, конвертации нет (число из тулбара
доезжает как есть); span'ы с собственным `font-size` тоже несут пункты как
есть, px конвертируется тем же ×0.75 (`_extract_size_pt`) — страховка на
случай внешней вставки, не путь тулбара. Начертание — только
...
Сегмент несёт и собственный кегль — `BlockSegment.size_pt` (не `None`,
когда пункт списка выделен на тулбаре целиком, §6): `render_block_segments`
подставляет его и в `base_size_pt` тела абзаца, и в МЕТКУ абзаца
(`_set_paragraph_mark_size`, `w:pPr/w:rPr/w:sz`, half-points) — Word
печатает маркер/номер именно меткой, а не текстовым run'ом, поэтому без
этого шага буллит/номер застыл бы на кегле стиля Normal независимо от
текста пункта. Метка выставляется каждому сегменту-пункту списка (даже без
собственного `size_pt` — тогда берётся `base_size_pt` хоста).
...
- **Превью текстблока — print-precise, supersedes B-22** (Task C, 2026-07):
  ...
  `preview-typography.css`) line-height — одинарный Word-интервал DOCX
  ...
  не редактор. Токен `--textblock-line-height` **удалён**: редактор
  (`.textblock-editor`, `textblock-content.css`) с тех пор сам перешёл на
  документный `--doc-line-height` (1.15) — разъехаться редактору и превью
  больше нечем, `--preview-print-line-height` (`preview-page.css`) — прямой
  алиас `--doc-line-height`. Правило CSS по-прежнему разбито на два уровня
  специфичности: базовый `.preview-textblock-content` (justify +
  `--doc-line-height`) — фолбэк для мест БЕЗ `.preview-sheet` (диалог истории
  версий, `PreviewTextBlockRenderer`, у него нет «листа»); print-precise
  `.preview-sheet .preview-textblock-content` (тот же интервал +
  `margin-bottom: 3pt`) перекрывает его внутри печатного листа за счёт
  специфичности селектора — значения теперь совпадают, разница только в
  отступе.
- **Втяжка списков предпросмотра — та же дыра дважды, теперь закрыта на
  четырёх поверхностях.** Правила `preview-typography.css` для `ol`/`ul`/`li`
  (втяжка, каскад маркеров по уровням) были селекторами вида `.preview ol`,
  а класс `.preview` висит ТОЛЬКО на inline-панели (`#preview`,
  `preview_panel.html`). Три другие поверхности рисуются теми же рендерами,
  но без этого класса: модальное меню предпросмотра (`#previewMenuBody`,
  `header_preview_button.html`), диалог истории версий
  (`.version-preview-ui`, `portal/acts-manager/version-preview.js`) и
  вкладка «Сравнение» того же диалога (`.diff-content`,
  `audit_log_dialog.html`) — `diff-renderer.js::_renderDiffTextBlock` гонит
  HTML текстблока через тот же `renderActContent` в `.diff-textblock`, а тот
  лежит вне `.version-preview-ui` (соседний блок в шаблоне). Списки во всех
  трёх попадали под глобальный сброс (`base/reset.css` обнуляет
  padding/margin всему): нулевая втяжка, маркер при
  `list-style-position: outside` уезжал за левый край контента (замер:
  `li.left == content.left`). Фикс — селекторы переписаны на
  `:is(.preview, .preview-sheet, .version-preview-ui, .diff-content)`;
  втяжка переведена в документный токен `--doc-list-indent` (36pt =
  `w:ind left` 720 твипов на уровень, `docx/numbering.py::_LIST_INDENT_STEP`)
  — общий с редактором (`textblock-content.css`), вместо прежних rem-токенов
  (`--spacing-2xl` в превью / `--spacing-xl` в редакторе), плававших вместе
  с плотностью зоны (18px в конструкторе, 24px в inline-превью, 0 в трёх
  других поверхностях). Особенность вкладки «Сравнение»: кегль текста внутри
  `.diff-textblock` остаётся интерфейсным (`font-size: 14px`,
  `line-height: 1.6`, `version-preview.css`), НЕ документным — сознательно,
  вкладка сравнения не претендует на печатную точность, ей нужна только
  видимость маркеров списка, а не соответствие вордовскому кеглю. **Правило
  на будущее**: документную типографику нельзя вешать на класс ОДНОЙ
  поверхности предпросмотра — их минимум четыре (inline-панель / модальное
  меню / диалог версий / вкладка «Сравнение» того же диалога), и `.preview`
  есть только у первой; новый селектор для листа/списка/блока предпросмотра
  обязан целиться в
  `:is(.preview, .preview-sheet, .version-preview-ui, .diff-content)`. Это
  уже второй случай ровно той же дыры — первый был с `line-height` (см.
  предыдущий пункт, базовое правило `.preview-textblock-content`); третий —
  гарнитура (следующий пункт), но её чинит другой приём: не список
  поверхностей `:is(...)`, а прицел в корни рендеров — список выше на
  гарнитуру механически не переносить.
- **Гарнитура документа была видна только на листе A4, теперь она токен и
  дошла до всех поверхностей правки/предпросмотра.** Замер вычисленного
  `font-family` ДО правки: `.textblock-editor` — интерфейсный sans;
  `.editable-table td`/`th` (таблицы конструктора) — sans (своего правила не
  было вовсе, наследовали от `body`); `.violation-textarea` (поля
  нарушения) — sans; диалог истории версий (`.version-preview-ui`)
  целиком — sans: текстблок, нарушение, таблица, шапка таблицы, название
  таблицы; вкладка «Сравнение» (`.diff-content`) — sans; сам лист A4 — Times
  ВЕЗДЕ, кроме `.preview-table-title` (название таблицы тянуло
  `--font-family-base`, то есть даже на печатном листе шло интерфейсным
  шрифтом). Документ печатается гарнитурой Times New Roman
  (`Fonts.main`, `app/domains/acts/formatters/docx/styles.py`), а на экране
  до правки её показывал ТОЛЬКО лист — хардкодом `font-family: "Times New
  Roman", serif` дважды в `preview-page.css` (сам лист и ячейки его таблиц).
  Не косметика: Times New Roman заметно уже системного sans, поэтому строка
  на экране обрывалась не там, где оборвётся в Word.

  Фикс — новый токен `--doc-font-family: "Times New Roman", serif` в
  документной секции `typography.css` (рядом с `--doc-font-size`; прежний
  UI-токен `--font-family-serif` поглощён им — его единственный
  потребитель, холст модалки предпросмотра, переведён на новый токен).
  Хардкоды в `preview-page.css` заменены на токен. Гарнитуру получили:
  поверхности правки — `.textblock-editor` (`textblock-content.css`),
  `.violation-textarea` (`violation-base.css`), `.editable-table th`/`td`
  (`table-base.css`), редакторы ячейки `textarea.cell-editor`
  (`table-editor.css`) и `td.editing input` (`table-states.css`, ради
  инварианта «вход в правку не меняет вид текста»); выход ОБЩИХ рендеров —
  `.preview-textblock, .preview-violation, .preview-table-wrapper` в
  `preview-typography.css` (это и чинит диалог версий, который листа не
  строит). Из `.preview-table th`/`.preview-table-title`
  (`preview-table.css`) интерфейсная гарнитура именно УБРАНА, а не
  переставлена на документную: эти элементы обязаны НАСЛЕДОВАТЬ гарнитуру
  поверхности (лист → Times, диалог версий → документная через
  `.preview-table-wrapper`, вкладка сравнения → интерфейсная).

  Два принципа на будущее. **Граница «документ vs интерфейс» проходит по
  рендеру, а не по поверхности** — целимся в КОРНИ РЕНДЕРОВ
  (`PreviewTextBlockRenderer`/`PreviewViolationRenderer`/`PreviewTableRenderer`),
  а не в корень поверхности: что выдал общий рендер — документ; что
  поверхность дорисовала вокруг (`.version-preview-heading`,
  `.version-preview-label` — их рисует `portal/acts-manager/version-preview.js`,
  а не общий рендер) — интерфейс. Повесь правило на `.version-preview-ui`
  целиком — в Times уехала бы и обвязка диалога. **Вкладка «Сравнение»
  исключена сознательно** — она эмитит собственные `.diff-*` классы и живёт
  по интерфейсному кеглю (`font-size: 14px`, `line-height: 1.6`,
  `version-preview.css`): инструмент сверки, а не отрисовка листа.

  Сторож — `tests/playwright/specs/32-document-parity.spec.ts`, describe
  «Гарнитура — документная, а не интерфейсная» (2 сценария: поверхности
  правки против листа; диалог версий — контент документный, обвязка
  интерфейсная, вкладка сравнения остаётся интерфейсной), плюс существующий
  тест входа в правку ячейки теперь сторожит и гарнитуру. Фальсификация
  проведена: на старом CSS оба новых сценария падают.
- **Та же дыра, тот же приём лечения — теперь для КЕГЛЯ таблиц общего
  рендера.** Гарнитура (предыдущий пункт) чинилась прицелом в корни рендеров;
  кегль таблиц общего рендера (`PreviewTableRenderer.create`, корень
  `.preview-table-wrapper`) страдал зеркальной болезнью: таблицы диалога
  истории версий (у него нет «листа» — только контент из общих рендеров)
  доставали печатные 9pt случайно, через интерфейсный `--font-size-sm`,
  который на портале прибит к 12px (`portal/layout/density.css`) — те же
  12px, что и `--doc-font-size-small` в pt. Совпадение немое: правка
  плотности портала увела бы таблицы от печатного кегля, а нарушения в том же
  диалоге (честно документные) остались бы на месте — разъезд был бы виден
  только глазами, ни один тест бы не упал. Фикс — то же правило прицела:
  новый селектор `.preview-table-wrapper .preview-table` (`preview-table.css`)
  задаёт `--doc-font-size-small`, достаёт и до листа A4, и до диалога версий,
  не задевая вкладку «Сравнение» (её `<table class="preview-table">`
  безобёрточная, рисуется `diff-renderer.js::_renderDiffTable`).
  **Вкладка «Сравнение» исключена сознательно** — тем же решением, что и по
  гарнитуре: инструмент сверки, а не отрисовка листа, живёт по интерфейсному
  кеглю. Заодно кегль подписи таблицы (`.preview-table-title`) в предпросмотре
  переведён с интерфейсного `--font-size-sm` на документный `--doc-font-size`
  (12pt) — подпись печатается наравне с телом акта; полное правило оформления
  подписи (начертание + подчёркивание по разделам) — §7.3
  [`data-model-acts.md`](data-model-acts.md).

  Сторожа два: страж-эталон `tests/test_document_typography_tokens.py`
  (CSS-токены `--doc-font-size`/`--doc-font-size-small` = питоновский `Sizes`
  из `docx/styles.py`, разъехаться могут только вручную — импорта Python в
  CSS нет) и ratchet `tests/test_document_css_font_size_ratchet.py` (кегль в
  документных CSS-зонах обязан приходить из `--doc-*`, а не из `--font-size-*`
  или литерала в pt; допущение для вкладки «Сравнение» явно перечислено в
  `ALLOWED_UI_TOKEN_FONT_SIZES` с обоснованием, а не молча пропущено).
  Фальсификация — `tests/playwright/specs/32-document-parity.spec.ts`, describe
  «Кегль таблиц предпросмотра — по поверхностям» (лист A4 и диалог версий на
  печатном кегле, вкладка «Сравнение» на интерфейсном).
...
делит список надвое; повторный
`indent` одного пункта не копит уровни и пункты без собственного текста
(БАГ-1); `indent` соседнего пункта уезжает в СУЩЕСТВУЮЩИЙ подсписок, а не
создаёт второй (БАГ-2); голый Tab вне
...
настроенный потолок глубины (спека читает его у редактора,
`_listLevelCeiling()`) и помечен там `aria-disabled`; дропдауны выравнивания/
списков открываются и применяют команду, не теряя выделение редактора
(BUG-3); см. §13).
Хелпер `hasOnlyValidListNesting` спеки проверяет ОБЕ половины инварианта:
у `ul`/`ol` только `<li>`-дети И ни один `<li>` не лежит прямо в другом `<li>`
(вторую половину порождал нативный outdent).
