...
`_indentListItem(li)` (`textblock-editor.js:1580`), а нативная команда для
...
ТОГО ЖЕ типа (`_sublistHost`, `:1602`, нумерация продолжается) либо новый.
Каретка переживает переезд через клон `Range` (`_captureCaretRange`/
`_restoreCaretRange`). Плата — нативный undo-стек Chromium своей мутации не
...
уровень (`_listLevel`, `textblock-editor.js:1543`) меньше потолка
