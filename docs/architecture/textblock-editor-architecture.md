...
(`:1526`), разбирающая случай «range заякорен на элементе»: Chrome при
...
**Собственный `indent` вместо нативного.** Нативный `execCommand('indent')`
...
`_indentListItem(li)` (`textblock-editor.js:1625`), а нативная команда для
`indent` не вызывается вовсе (`outdent` остался нативным + нормализация):
...
пункт, а Tab по нескольким строкам — только первую (нативная команда двигала
всё выделение, и `outdent`, оставшийся нативным, двигает его до сих пор).
Пункты обрабатываются в порядке документа: следующий видит предыдущий уже
переехавшим и попадает в тот же подсписок.

Каретка переживает переезд снимком ПЛОСКИМИ значениями (узел + смещение,
`_captureCaretRange`/`_restoreCaretRange`, `:1668`), а не клоном `Range`:
...
(`textblock-core.js:302`) ДО исполнения команды проверяет для
...
`_normalizeListNesting(root)` (`:1658`) приводит разметку к валидной по трём
правилам: (1) `ul|ol`, чей родитель `ul|ol`, переезжает внутрь ПРЕДЫДУЩЕГО
...
`:1690`) — иначе второй `ol` нумеровался бы с начала и в превью, и в DOCX;
...
(`_normalizeListNestingHtml`, `:1717`). Две точки вызова: `execCommand` — после
ЛЮБОЙ `LIST_STRUCTURE_CMDS` (`indent`/`outdent`/`insertUnorderedList`/
`insertOrderedList`, `textblock-core.js:66`), ДО `saveContent`; и БЕЗУСЛОВНО в
`saveContent` (`:166-167`) — правка могла
...
(живое поведение Chromium под многоуровневыми списками, 12 сценариев: Tab/
Shift+Tab меняют уровень пункта и дают валидную вложенность; повторный
