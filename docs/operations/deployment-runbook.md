...
- [ ] **`.env` сверен с `.env.prod`** (шаблона `.env.example` больше нет: ПРОМ — `.env.prod`, DEV — `.env.dev`). После предыдущего деплоя в `.env.prod` мог появиться обязательный ключ или поменяться дефолт. Команда быстрой сверки на Windows PowerShell:
  ...
  ```
  Особо проверить: `AUTH__*`, `REDIS__*`, `CHAT__*`, `ACTS__*`, `OBSERVABILITY__*`, `SECURITY__*`. Канал к внешнему ИИ-агенту настраивается префиксом `CHAT__AGENT_CHANNEL__*` (`TABLE_NAME` — имя bus-таблицы целиком, дефолт кода `chat_agent_messages_bus`, в `.env.prod` — `agent_conversation_messages`; `SCHEMA_NAME` — пусто → схема чата → основная; `POLL_MIN_INTERVAL_SEC=2.0`, `POLL_MAX_INTERVAL_SEC=10.0`, `POLL_BACKOFF_MULTIPLIER=1.5`, `ANSWER_TIMEOUT_SEC=1800`, `CLAIM_TIMEOUT_SEC=1800`, `MAX_BLOCK_TEXT_SIZE=262144`); лимит одновременных запросов — `CHAT__MAX_PARALLEL_STREAMS_PER_USER` (default 3).
...
- [ ] **Схема БД соответствует коду**. Если с прошлого деплоя менялась `schema.sql` любого домена — накатаны все файлы миграций `docs/migrations/<дата>-*.md`, появившиеся с прошлого деплоя (по порядку дат), либо БД пересоздана, если изменение ломающее (`docs/migrations/drop-all-tables.md`, см. §3); startup-диагностика дрейфа колонок (§6.5.4 в [`database.md`](../guides/database.md)) не сыпет WARNING'ами.
- [ ] **Внешний ИИ-агент жив**. Если деплой завязан на форварды в «Базу знаний ОАРБ» — убедиться, что внешний worker читает bus-таблицу из `CHAT__AGENT_CHANNEL__TABLE_NAME` (вопросы со `status='pending'`/`role='user'`) и пишет ответы туда же. Без него форварды будут висеть до `CHAT__AGENT_CHANNEL__CLAIM_TIMEOUT_SEC` (1800 сек, фаза `pending`) / `ANSWER_TIMEOUT_SEC` (1800 сек, фаза `processing`) и финализироваться как ошибка таймаута.
...
3. **Diagnostics (Wave 1).** Требует роль `Админ` (`ADMIN__USER_DIRECTORY__DEFAULT_ADMINS`):
   ```bash
...
Автоматических апгрейд-миграций нет: `create_tables_if_not_exist` создаёт **только отсутствующие** таблицы, не дописывает колонки/индексы в существующие. Что делать при изменении схемы:

- **Расширяющее изменение** (новая колонка, индекс, констрейнт) — накатить руками файл миграции `docs/migrations/<дата>-<таблица>-<поле>.md`: внутри готовые скрипты для PG и GP, раздел «Проверка» и указание, кому миграция нужна. Порядок — по дате в имени. Формат и правила — §6.5.5 в [`database.md`](../guides/database.md).
- **Ломающее изменение** (переименование, смена типа, удаление колонки) — файла миграции нет, БД пересоздаётся: `docs/migrations/drop-all-tables.md` (дроп строго в обратном порядке FK) и рестарт приложения.

Для имитации внешнего агента см. [`external-agent-imitation.sql`](../integrations/external-agent-imitation.sql).
