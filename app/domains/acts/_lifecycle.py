...
def get_format_thread_pool() -> ThreadPoolExecutor:
    ...
    в app.state, затем в module-level _thread_pool.
    """
    executor: ThreadPoolExecutor | None = _thread_pool
    if executor is None:
...
async def on_startup(app: FastAPI) -> None:
    ...
    # каноническим владельцем; _thread_pool — удобный геттер без Request.
    _thread_pool = app.state.executor
...
# Обновляется lifecycle-хуками; не использовать напрямую вне get_format_thread_pool().
