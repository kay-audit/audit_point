...
# Отступ на уровень вложенности (общий для TXT и MD — см. _convert_lists).
#
# 4 пробела, не 2: ширина маркера — 2 у «- » и 3 у «N. » (content indent,
# позиция начала текста пункта). Чтобы вложенный список распознавался
# CommonMark-совместимым рендерером как ПОТОМОК родительского пункта (а не
# его сиблинг с перенумерацией), отступ вложенного маркера должен быть >=
# content indent родителя. При unit=2 это выполняется для «- » (2>=2), но НЕ
# для «N. » (2<3) — вложенность разваливается. При unit=4: «- » получает
# запас +2 сверх content indent, «N. » (до 2-значных номеров, content indent
# 3-4) — запас +1..0, тоже безопасно (код-блок начинается от +4 ОТНОСИТЕЛЬНО
# content indent контейнера, а не от начала строки).
# Кумулятивно на глубину: между СОСЕДНИМИ уровнями всегда одна и та же
# дельта — unit (4), не зависящая от глубины (0->4->8->12 для уровней
# 0/1/2/3). Проверка «нужный отступ >= content indent родителя» на каждом
# уровне сравнивает именно эту дельту с content indent НЕПОСРЕДСТВЕННОГО
# родителя на этом уровне — а не с абсолютной глубиной, поэтому глубина 3
# не отличается от глубины 1: та же арифметика повторяется на каждом шаге.
_LIST_INDENT_UNIT = "    "
...
def _convert_lists(content: str, *, blank_line_around: bool = False) -> str:
    ...
    Вложенность БОЛЬШЕ НЕ СПЛЮЩИВАЕТСЯ: каждый уровень получает отступ
    (``_LIST_INDENT_UNIT`` на уровень, level = глубина списка, 0-based) —
    общее для TXT и MD. Маркер не меняется по уровню (тот же «- »/«N. », что
    и на уровне 0) — для .txt/.md это plain-text/Markdown-выгрузка, а не
    визуальный рендер «как в Word» (та схема — DOCX/CSS, см. inline.py и
    numbering.py); Markdown-нумерованный список к тому же поддерживает только
    цифровые маркеры.

    Счётчик своего списка (``list_stack``) стартует с 0 при КАЖДОМ открытии
    <ul>/<ol>, в том числе вложенном, — нумерация вложенного списка всегда
    начинается заново, независимо от соседних списков того же уровня.
        ...
        if content[i] == "<":
            ...
            if m:
                ...
                if _LI_OPEN_RE.match(tag) and list_stack:
                    ...
                    indent = _LIST_INDENT_UNIT * (len(list_stack) - 1)
                    marker = "-" if kind == "ul" else f"{count}."
                    out.append(f"<div>{indent}{marker} ")
                    li_open = True
                    i = m.end()
                    continue
...
class HTMLUtils:
    ...
    def html_to_markdown(content: str) -> str:
        ...
        - <ul>/<ol>/<li> -> markdown-список («- пункт» / «N. пункт»),
          обрамлённый пустой строкой от соседнего контента; вложенные уровни —
          отступ 4 пробела на уровень (см. _LIST_INDENT_UNIT; маркер тот же,
          что на уровне 0: буква/римское сломали бы markdown-синтаксис
