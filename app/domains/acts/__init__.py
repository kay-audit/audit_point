        ...
        nav_items=[
            ...
            NavItem(
                ...
                icon_svg=(
                    ...
                    # горизонтальный разделитель шапки
                    ...
                    'stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'
                    # точки-даты в две строки
                    '<rect x="6.5" y="13" width="1.5" height="1.5" fill="currentColor"/>'
                    '<rect x="11" y="13" width="1.5" height="1.5" fill="currentColor"/>'
                    '<rect x="15.5" y="13" width="1.5" height="1.5" fill="currentColor"/>'
                    '<rect x="6.5" y="17" width="1.5" height="1.5" fill="currentColor"/>'
                    '<rect x="11" y="17" width="1.5" height="1.5" fill="currentColor"/>'
                    '<rect x="15.5" y="17" width="1.5" height="1.5" fill="currentColor"/>'
                ),
                order=11,
...
chat_system_prompt=(
            ...
            "При вызове acts.add_processes_to_act / acts.modify_act_tree / "
            ...
            "\n"
            "ВАЖНО: если в контексте есть «Текущий открытый акт» — НЕ "
            "спрашивай «в какой акт добавить», «какой акт» и т.п. — "
            "передавай act_id прямо в tool. Спрашивать нужно ТОЛЬКО если "
            "пользователь явно сказал «в другой акт» / «в КМ-99-XXXXX» / "
            "«в акт по выбору», а current_act_id в контексте не указан.\n"
            "\n"
            "Если в контексте есть «Выбранный блок» — пользователь "
            "вызвал AI-ассистента через контекстное меню конкретного "
            "узла. По умолчанию операции модификации (add_item, add_table, "
            "add_textblock, add_violation) делай для ЭТОГО блока как "
            "родителя. Если пользователь явно просит другой блок или корень "
            "(например, «добавь в раздел 5») — используй parent_id явно.\n"
            "\n"
            "## Краткий стиль ответов при модификации структуры\n"
            "Когда пользователь дал задачу («добавь подпункт», «удали "
            "таблицу», «вставь таблицу рисков», «запиши нарушение»):\n"
            "- СРАЗУ вызывай acts.modify_act_tree с нужной операцией. "
            "НЕ перечисляй в ответе доступные операции и возможности — "
            "пользователь уже знает, что умеет ассистент (контекстное "
            "меню в конструкторе показывает то же самое).\n"
            "- В ответе — только краткое подтверждение успеха "
            "(«Готово. Добавлен подпункт 2.3 «Описание процесса»»). "
            "Без длинных списков, без пояснений «что я могу сделать».\n"
            "- Если задача неоднозначна (например, «добавь таблицу» без "
            "указания типа риска или подпункт без названия) — задай "
            "ОДИН короткий уточняющий вопрос. Не вызывай modify_act_tree "
            "с догадками.\n"
            "- Если пользователь говорит «сделай что-нибудь», "
            "«предложи изменения» — тогда подскажи, что обычно делают "
            "в этом блоке. Но если задача конкретная — просто делай.\n"
            "\n"
            "## ОБЯЗАТЕЛЬНЫЕ поля в modify_act_tree.operations\n"
            "tool modify_act_tree отклоняет операции без обязательных полей. "
            "Перед каждым вызовом проверяй, что в объекте есть всё нужное:\n"
            "- add_item: parent_id (id узла '2' ИЛИ human-readable number '2' "
            "или '2.1'), label.\n"
            "- add_sibling: node_id, label.\n"
            "- add_textblock: parent_id, label.\n"
            "- add_table: parent_id, label, kind ('regular' / 'metrics' / "
            "'mainMetrics' / 'regularRisk' / 'operationalRisk' / 'taxRisk' / "
            "'otherRisk').\n"
            "- fill_table: table_id (tableId из add_table ИЛИ number "
            "таблицы-узла вида 'Таблица 1.2'), grid (2D-массив). "
            "Опционально: mode ('replace' по умолчанию ИЛИ 'append_rows'), "
            "col_widths ([120,80,100]). В режиме replace первая строка = "
            "заголовки (isHeader=True), остальные = данные. В режиме "
            "append_rows все строки = данные (заголовки уже есть). Формат "
            "grid: compact (['Товар','Кол-во']) или full "
            "([{content,isHeader,colSpan,rowSpan,originRow,originCol}]).\n"
            "- add_violation: parent_id, label, violated, established.\n"
            "- add_process_mining: ничего не нужно (добавляется в корень).\n"
            "- delete_node / move_node: node_id.\n"
            "Если поле отсутствует — tool вернёт ошибку «X обязателен» и "
            "ничего не сохранит. Не вызывай tool с пустыми/неполными полями — "
            "если чего-то не хватает, спроси у пользователя.\n"
            "Если в контексте есть «Выбранный блок: id=X, number=Y» — "
            "parent_id по умолчанию = X (id узла) или Y (number). Оба формата "
            "tool принимает.\n"
            "\n"
            ...
            "- fill_table — заполнить или дополнить существующую таблицу. "
            "table_id = tableId из add_table ИЛИ human-readable number "
            "таблицы ('Таблица 1.2'). grid: 2D-массив — compact "
            "([['Товар','Кол-во'],['Ручка',5]]) или full "
            "([{content,isHeader,colSpan,rowSpan,...}]). mode='replace' "
            "(по умолчанию) — заменяет таблицу целиком (первая строка = "
            "заголовки); mode='append_rows' — добавляет строки в конец без "
            "изменения заголовков. Опционально col_widths=[120,80,100] — "
            "ширины колонок. Если пользователь говорит «заполни таблицу 5.2.3», "
            "«добавь строки в таблицу», «впиши данные в таблицу операционного "
            "риска» — ОБЯЗАТЕЛЬНО вызови fill_table, НЕ говори пользователю "
            "делать это руками. Если таблицы ещё нет — сначала add_table, "
            "затем fill_table (можно одним вызовом modify_act_tree с "
            "массивом operations).\n"
