...
ChatTool(
        ...
        ChatTool(
            ...
            description=(
                ...
                "нарушение, раздел Process Mining; удалить узел; переместить "
                "узел. Этот tool делает то же, что пользователь может сделать "
                "через контекстное меню в конструкторе («Добавить подпункт», "
                ...
                "Process Mining», «заполни таблицу 5.2.3 — колонки Товар/Кол-во, "
                "данные: Ручка/5, Карандаш/12», «добавь строки в таблицу 5.2.3». "
                "Параметр operations — список операций (можно комбинировать в "
                "одном вызове, например добавить сразу 5 пунктов и 1 таблицу, "
                "или add_table + fill_table одним вызовом). Права: "
                "Куратор/Руководитель/Редактор/Админ (Участник без права edit "
                "видит понятный отказ). parent_id и node_id принимают как "
                "полный id узла, так и human-readable number ('2', '5.1.2'). "
                "При успехе возвращает client_action с переходом на "
                "/constructor?act_id={act_id} и сводку с id/number каждого "
                "созданного/удалённого узла."
            ),
            parameters=[
                ChatToolParam(
                    "act_id", "integer",
