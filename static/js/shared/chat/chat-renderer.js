...
/**
     ...
     *
     * Считается от ``import.meta.url`` этого модуля, а не хардкодится:
     * модуль загружается по версионированному пути
     * (``/static/v<версия>/js/shared/chat/``), поэтому иконки получают ту же
     * версию и обновляются после релиза без Ctrl+Shift+R. Хардкод
     * ``/static/icons/chat/`` версию не наследовал бы — иконки грузятся
     * как картинки, а не как модули.
     *
     * Фолбэк на абсолютный путь — на случай окружения без ``import.meta``
     * (например, если модуль когда-нибудь загрузят не как ES-модуль).
     * @private
     */
    _ICONS_BASE_URL: (() => {
        try {
            return new URL('../../../icons/chat/', import.meta.url).href;
        } catch {
            return '/static/icons/chat/';
        }
    })(),
