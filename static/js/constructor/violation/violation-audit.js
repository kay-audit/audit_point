 ...
 *
 * Производительность: снимок лёгкий по построению — байты картинок живут в
 * таблице act_images, а блок несёт только ссылку. Отпечаток картинки — её id
 * плюс метаданные (тип/image_id/подпись/имя файла/ширина). Сравнение —
 * строковое равенство нормализованного отпечатка нарушения; замена картинки
 * меняет image_id и потому фиксируется.
 *
...
export class ViolationAudit {
     ...
     * image — id+ссылка на картинку+подпись+имя+ширина; table — id+сетка
     * content-ов (без span-метаданных:
